...
def downsample_processed(processed_root: Path, dest_root: Path, fraction: float, seed: int, copy: bool=False, force: bool=False) -> Path:
    ...
    for ((split, label), items) in sorted(grouped.items()):
        ...
        for src_path in selected:
            ...
            manifest_rows.append({'split': split, 'label': label, 'dataset_name': meta.get('dataset_name', ''), 'raw_path': meta.get('raw_path', str(src_path)), 'session_name': stem, 'bin_path': str(dst_bin)})
            total_linked += 1
...
def main():
    ...
    downsample_processed(processed_root=processed_root, dest_root=dest_root, fraction=args.fraction, seed=args.seed, copy=args.copy, force=args.force)
