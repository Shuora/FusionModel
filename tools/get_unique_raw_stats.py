...
def get_unique_stats(manifest_path):
    ...
    # Label -> {Train: set(raw_paths), Test: set(raw_paths)}
    unique_samples = {}
    for item in data:
        label = item['label']
        split = item['split']
...
tasks = {
    "Binary": "ProcessedData/binary_benign_vs_malicious/metadata/manifest.json",
    "USTC": "ProcessedData/ustc_multiclass/metadata/manifest.json",
    "MTA": "ProcessedData/mta_multiclass/metadata/manifest.json",
    "MFCP": "ProcessedData/mfcp_multiclass/metadata/manifest.json"
}
