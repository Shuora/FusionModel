...
def measure_latency(model, dummy_inputs, device, num_runs=100, warm_up=10):
    ...
    return (np.mean(latencies) * 1000, np.std(latencies) * 1000)
...
def main():
    ...
    num_classes = 10
    models = {'DeepPacket (CNN)': DeepPacket(num_classes=num_classes), 'LSTM': LSTMClassifier(num_classes=num_classes), 'MobileViT-only': MobileViTOnly(num_classes=num_classes), 'Fusion-Attention (Ours)': initialize_fusion_model(num_classes=num_classes, fusion_mode='attention'), 'Fusion-Concat (Ablation)': initialize_fusion_model(num_classes=num_classes, fusion_mode='concat')}
    ...
    for (name, model) in models.items():
        ...
        if HAS_FVCORE:
            try:
                ...
                flops = flops_analyzer.total() / 1000000000.0
