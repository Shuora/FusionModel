...
SOURCE = os.path.abspath(r"C:\Repositories\Traffic\CharBERT-MobileViT\outputs\logs\concat_20260205_235803.log")
LOGDIR = os.path.dirname(SOURCE)
BACKUP_TS = datetime.now().strftime("%Y%m%d_%H%M%S")

DONE_RE = re.compile(r"\bdone\b.*log=(?P<path>\S+)", re.IGNORECASE)
START_RE = re.compile(r"\bstart\b", re.IGNORECASE)
...
def main():
    ...
    sections = []  # each: {'start_idx','end_idx','lines','target'}
    in_section = False
    buf = []
    start_idx = None
    ...
    for idx, line in enumerate(lines):
        ...
        if in_section:
            ...
            if m:
                ...
                # Only keep basename so we place files into same dir
    ...
    if in_section and buf:
        ...
        sections.append({
            ...
            'lines': buf.copy(),
            'target': misc_target,
            'target_basename': os.path.basename(misc_target),
        })
    ...
    # Process sections: append to targets (skip if target equals source)
    for i, s in enumerate(sections, 1):
        ...
        entry = {
            ...
            'end_idx': s['end_idx'],
            'target': target,
            'target_basename': s['target_basename'],
            'appended': False,
            'reason': '',
        }

        # If target equals SOURCE, skip (already present)
        ...
        if os.path.exists(target):
            ...
            # Dedup check: search for start_line in target
