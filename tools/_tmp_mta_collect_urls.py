...
END = date(2023, 2, 28)
UA = 'Mozilla/5.0'
FAMILY_PATTERNS = {'Dridex': re.compile('\\\\bdridex\\\\b', re.I), 'Emotet': re.compile('\\\\bemotet\\\\b', re.I), 'Hancitor': re.compile('\\\\bhancitor\\\\b', re.I), 'IcedID': re.compile('\\\\biced[\\\\s-]?id\\\\b', re.I), 'Qakbot': re.compile('\\\\bqakbot\\\\b|\\\\bqbot\\\\b', re.I), 'Trickbot': re.compile('\\\\btrickbot\\\\b', re.I), 'Ursnif': re.compile('\\\\bursnif\\\\b|\\\\bgozi\\\\b|\\\\bisfb\\\\b', re.I)}
DAY_PAGE_RE = re.compile('/(\\\\d{4})/(\\\\d{2})/(\\\\d{2})/index\\\\d*\\\\.html$')
ZIP_RE = re.compile('pcap\\\\.zip', re.I)
