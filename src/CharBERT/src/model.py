...
def _build_char_lookup_table(vocab_size: int, char_vocab: str) -> Tuple[torch.Tensor, int]:
    ...
    for token_id in range(vocab_size):
        ...
        elif token_id < 256 and 32 <= token_id <= 126:
...
class CharBERT(nn.Module):
    ...
    def __init__(self, vocab_size: int, hidden_size: int, num_layers: int, num_heads: int, dropout: float, num_labels: int, max_len: int, mode: str='legacy', char_vocab: str='hex', char_emb_dim: int=32, char_cnn_channels: int=64, char_fusion: str='gated', char_fusion_layers: str='all'):
        super().__init__()
        ...
        if mode == 'legacy':
            ...
            self.encoder = None
            self.encoder_layers = nn.ModuleList([nn.TransformerEncoderLayer(d_model=hidden_size, nhead=num_heads, dim_feedforward=hidden_size * 4, dropout=dropout, batch_first=True) for _ in range(num_layers)])
            ...
            if char_fusion == 'gated':
                self.fusion_gate = nn.Sequential(nn.Linear(hidden_size * 2, hidden_size), nn.ReLU(), nn.Linear(hidden_size, hidden_size), nn.Sigmoid())
...
def build_model(cfg, num_labels: int):
    return CharBERT(vocab_size=cfg.vocab_size, hidden_size=cfg.hidden_size, num_layers=cfg.num_layers, num_heads=cfg.num_heads, dropout=cfg.dropout, num_labels=num_labels, max_len=cfg.max_len, mode=getattr(cfg, 'mode', 'legacy'), char_vocab=getattr(cfg, 'char_vocab', 'hex'), char_emb_dim=getattr(cfg, 'char_emb_dim', 32), char_cnn_channels=getattr(cfg, 'char_cnn_channels', 64), char_fusion=getattr(cfg, 'char_fusion', 'gated'), char_fusion_layers=getattr(cfg, 'char_fusion_layers', 'all'))