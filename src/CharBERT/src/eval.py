...
def evaluate_predictions(y_true: List[int], y_pred: List[int], labels: List[str]):
    ...
    return {'precision_macro': precision, 'recall_macro': recall, 'f1_macro': f1, 'accuracy': acc, 'report': report, 'cm': confusion_matrix(y_true, y_pred, labels=list(range(len(labels))))}
