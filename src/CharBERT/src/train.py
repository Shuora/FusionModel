...
def validate(model: nn.Module, loader: DataLoader, device: torch.device, logger, id_to_label: Dict[int, str]) -> Dict[str, float]:
    ...
    logger.info('验证集分类报告:\n' + report_text)
    logger.info('验证集宏指标: 精确率 {:.4f}, 召回率 {:.4f}, F1 {:.4f}, 准确率 {:.4f}'.format(metrics['precision_macro'], metrics['recall_macro'], metrics['f1_macro'], metrics['accuracy']))
    return {'loss': epoch_loss, 'acc': epoch_acc, 'precision_macro': metrics['precision_macro'], 'recall_macro': metrics['recall_macro'], 'f1_macro': metrics['f1_macro'], 'all_true': all_true, 'all_pred': all_pred, 'cm': metrics['cm']}
...
def main():
    ...
    (train_loader, test_loader, label_to_id, id_to_label) = create_dataloaders(cfg.train_dir, cfg.test_dir, cfg.max_len, cfg.batch_size, cfg.num_workers, cfg.label_list)
    ...
    if cfg.eval_only:
        ...
        logger.info('最终准确率 {:.4f}, 宏精确率 {:.4f}, 宏召回率 {:.4f}, 宏F1 {:.4f}'.format(final_metrics['acc'], final_metrics['precision_macro'], final_metrics['recall_macro'], final_metrics['f1_macro']))
        return
