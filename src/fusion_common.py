...
MTA_CLASS_SIGNATURES = (frozenset(['dridex', 'emotet', 'hancitor', 'qakbot', 'trickbot', 'ursnif']), frozenset(['dridex', 'emotet', 'hancitor', 'icedid', 'qakbot', 'trickbot', 'ursnif']))
MFCP_CLASS_SIGNATURES = (frozenset(['artemis', 'dridex', 'pua', 'trickbot', 'ursnif']), frozenset(['artemis', 'cobalt', 'dridex', 'pua', 'trickbot', 'ursnif']), frozenset(['artemis', 'cobalt', 'dridex', 'pua', 'emotet', 'ursnif']))
MFCP_PAIR_CLASS_NAMES = ('artemis', 'ursnif')
KNOWN_TASK_NAMES = {'binary_benign_vs_malicious', 'ustc_multiclass', 'mta_multiclass', 'mfcp_multiclass'}
...
def detect_stacking_special_tasks(*, train_classes: List[str], train_image_dir: Optional[Union[str, os.PathLike]]=None, train_pcap_dir: Optional[Union[str, os.PathLike]]=None, explicit_task_name: str='') -> Tuple[bool, bool]:
...
def select_confusion_pair(*, labels: np.ndarray, preds: np.ndarray, preferred_pair: Optional[Tuple[int, int]]=None) -> Optional[Tuple[int, int]]:
...
def setup_logging(log_file: Optional[Union[str, os.PathLike]]=None, *, level: int=logging.INFO, force: bool=False) -> Path:
    ...
    kwargs = dict(level=level, format='%(asctime)s - %(levelname)s - %(name)s - %(message)s', handlers=[logging.FileHandler(str(log_file), encoding='utf-8'), logging.StreamHandler()])
...
def _is_flat_dataset_layout(dataset_dir: Union[str, os.PathLike]) -> bool:
    root = Path(dataset_dir)
    required_dirs = [root / 'image_data' / 'Train', root / 'image_data' / 'Test', root / 'pcap_data' / 'Train', root / 'pcap_data' / 'Test']
    return all((d.exists() and d.is_dir() for d in required_dirs))

def resolve_dataset_dirs(dataset_root: Union[str, os.PathLike], dataset_name: Optional[str]=None) -> tuple[str, str, str, str, str]:
    root = Path(dataset_root)
    if not root.exists():
        raise FileNotFoundError(f'dataset_root does not exist: {root}')
    if not root.is_dir():
        raise NotADirectoryError(f'dataset_root is not a directory: {root}')
    if _is_flat_dataset_layout(root) and (not dataset_name or dataset_name == root.name):
        dataset_dir = root
        resolved_name = root.name
    else:
        available = list_available_datasets(root)
        if not available:
            raise FileNotFoundError(f'No dataset directories found under: {root}. If dataset_root is itself a dataset, ensure it contains image_data/Train, image_data/Test, pcap_data/Train, pcap_data/Test.')
        resolved_name = dataset_name if dataset_name else available[0]
        dataset_dir = root / resolved_name
        if not dataset_dir.exists() or not dataset_dir.is_dir():
            raise FileNotFoundError(f"Dataset '{resolved_name}' not found under {root}. Available datasets: {available}. For flat layout, use --dataset_root {root} and leave --dataset_name empty.")
    ...
    if missing:
        ...
        raise FileNotFoundError(f"Dataset directory structure is incomplete for '{resolved_name}'. Missing directories: {missing}")
    return (str(train_image_dir), str(train_pcap_dir), str(test_image_dir), str(test_pcap_dir), resolved_name)
...
def resolve_task_dataset_dirs(processed_root: Union[str, os.PathLike], task_name: str, image_mode: str='rgb') -> tuple[str, str, str, str, str]:
    ...
    missing = [str(d) for d in required_dirs if not d.exists() or not d.is_dir()]
    if missing:
        raise FileNotFoundError(f'Task dataset structure is incomplete for {task_name}. Missing directories: {missing}')
    return (str(train_image_dir), str(train_pcap_dir), str(test_image_dir), str(test_pcap_dir), task_name)
...
def prepare_run_output_dir(output_dir: Path, run_name: str) -> Path:
    ...
    safe_name = re.sub('[^a-zA-Z0-9._-]+', '_', str(run_name).strip())
...
def build_run_artifact_paths(run_dir: Path) -> dict:
    ...
    return {'train_log': run_dir / 'train.log', 'metrics_json': run_dir / 'metrics.json', 'epoch_metrics_csv': run_dir / 'epoch_metrics.csv', 'metrics_curve': run_dir / 'metrics_curve.png', 'confusion_matrix': run_dir / 'confusion_matrix.png', 'attention_curve': run_dir / 'attention_curve.png', 'report_md': run_dir / 'report.md', 'model': run_dir / 'fusion_model.pth', 'base_model': run_dir / 'fusion_model_base.pth'}
...
class EarlyStopping:
    ...
    def __init__(self, patience: int=4, min_delta: float=0.0, restore_best_weights: bool=True, mode: str='min'):
...
def _resolve_early_stop_mode(early_stop_metric: str, early_stop_mode: str) -> str:
    metric_to_mode = {'val_loss': 'min', 'val_acc': 'max', 'val_f1': 'max'}
    ...
    if early_stop_mode != expected_mode:
        raise ValueError(f"early_stop_mode={early_stop_mode} 与 early_stop_metric={early_stop_metric} 不一致，请使用 '{expected_mode}' 或 'auto'")
...
def build_temporal_pcap_token_ids(packet_records: list[dict], max_pcap_length: int, *, pad_token: int=256, cls_token: int=257, sep_token: int=258) -> list[int]:
    ...
    return token_ids
_TEMPORAL_PACKET_HEADER_RE = re.compile('^<PKT dir=(?P<direction>\\d+) len=(?P<length>\\d+) dt=(?P<delta>[0-9]+(?:\\.[0-9]+)?)>$')

def _decode_temporal_packet_header(header_tokens: list[int]) -> dict[str, float | int]:
    header_text = bytes((int(token) for token in header_tokens if 0 <= int(token) <= 255)).decode('ascii', errors='ignore')
    match = _TEMPORAL_PACKET_HEADER_RE.match(header_text)
    if match:
        return {'direction': int(match.group('direction')), 'packet_length': int(match.group('length')), 'delta_t': float(match.group('delta'))}
...
def _extract_temporal_packet_records(input_ids: list[int], pad_id: int) -> list[dict[str, int | float]]:
    ...
    while idx <= n - len(header_prefix):
        ...
        records.append({'header_start': header_start, 'header_end': header_end, 'payload_start': payload_start, 'payload_end': footer_start, 'direction': header_meta['direction'], 'packet_length': header_meta['packet_length'], 'delta_t': header_meta['delta_t']})
...
def _build_temporal_packet_meta_tensor(record: dict[str, int | float], *, device, dtype) -> torch.Tensor:
    ...
    meta = torch.tensor([direction, math.log1p(max(packet_length, 0.0)) / 10.0, math.log1p(max(delta_t, 0.0)) / 10.0], device=device, dtype=dtype)
    return meta
...
def pool_temporal_packet_blocks(encoded: torch.Tensor, input_ids: torch.Tensor, pad_id: int, attention_mask: Optional[torch.Tensor]=None) -> Optional[torch.Tensor]:
...
class FusionDataset(Dataset):
    ...
    def __init__(self, image_dir: str, pcap_dir: str, transform=None, max_pcap_length: int=784, *, use_index_cache: bool=True, rebuild_index_cache: bool=False, use_temporal_sidecar: bool=True, image_mode: str='rgb'):
        ...
        self.classes = sorted([name for name in os.listdir(image_dir) if os.path.isdir(os.path.join(image_dir, name))])
        ...
        for (class_idx, class_name) in enumerate(self.classes):
            ...
            if os.path.exists(image_class_dir) and os.path.exists(pcap_class_dir):
                image_files = [e.name for e in os.scandir(image_class_dir) if e.is_file() and e.name.lower().endswith(('.png', '.jpg', '.jpeg'))]
                pcap_files = [e.name for e in os.scandir(pcap_class_dir) if e.is_file() and (e.name.lower().endswith(('.bin', '.pcap')) or 'pcap' in e.name.lower())]
                pcap_exact_index = {}
                pcap_norm_index = {}
                ...
                for img_file in image_files:
                    ...
                    matching_pcap = self._find_matching_pcap(img_base=img_base, pcap_files=pcap_files, pcap_exact_index=pcap_exact_index, pcap_norm_index=pcap_norm_index)
                    if matching_pcap:
                        self.samples.append((os.path.join(image_class_dir, img_file), os.path.join(pcap_class_dir, matching_pcap), class_idx))
    ...
    def _normalize_stem(stem: str) -> str:
        return re.sub('[^a-z0-9]+', '', stem.lower())
    ...
    def _save_index_cache(self, cache_path: str) -> None:
        try:
            payload = {'cache_version': int(self.CACHE_VERSION), 'image_dir': os.path.abspath(self.image_dir), 'pcap_dir': os.path.abspath(self.pcap_dir), 'classes': self.classes, 'samples': self.samples}
    ...
    @classmethod
    def _find_matching_pcap(cls, img_base: str, pcap_files: List[str], pcap_exact_index: dict, pcap_norm_index: dict) -> Optional[str]:
                    ...
                    try:
                        ...
                        if version != 1:
                            ...
                            if isinstance(packets, list) and packets:
                                tokens = build_temporal_pcap_token_ids(packets, max_pcap_length=max_len, pad_token=pad_token, cls_token=cls_token, sep_token=sep_token)
...
def load_fusion_data(image_dir: str, pcap_dir: str, batch_size: int=64, image_size: int=28, max_pcap_length: int=784, *, num_workers: int=4, pin_memory: bool=False, persistent_workers: bool=False, prefetch_factor: int=2, use_index_cache: bool=True, rebuild_index_cache: bool=False, is_train: bool=True, balance_mode: str='none', selected_groups: Optional[List[str]]=None, use_temporal_sidecar: bool=True, image_mode: str='rgb'):
    logger.info('加载融合数据 - 图像目录: %s, Pcap目录: %s', image_dir, pcap_dir)
    transform = transforms.Compose([transforms.Resize((image_size, image_size)), transforms.ToTensor(), transforms.Lambda(convert_grayscale_to_rgb)])
    ...
    if os.path.abspath(image_dir) == os.path.abspath(pcap_dir):
        ...
        if selected_groups:
            ...
            if not grouped_pairs:
                raise FileNotFoundError(f'--cic_group={selected_groups} 未匹配到任何分组。可用分组: {sorted(set(all_group_names))}')
    if grouped_pairs:
        ...
        datasets = [FusionDataset(img_dir, p_dir, transform, max_pcap_length, use_index_cache=use_index_cache, rebuild_index_cache=rebuild_index_cache, use_temporal_sidecar=use_temporal_sidecar, image_mode=image_mode) for (img_dir, p_dir) in grouped_pairs]
        ...
        dataset = MergedFusionDataset(merged_samples, all_class_names, transform=transform, max_pcap_length=max_pcap_length, use_temporal_sidecar=use_temporal_sidecar, image_mode=image_mode)
    else:
        dataset = FusionDataset(image_dir, pcap_dir, transform, max_pcap_length, use_index_cache=use_index_cache, rebuild_index_cache=rebuild_index_cache, use_temporal_sidecar=use_temporal_sidecar, image_mode=image_mode)
    dl_kwargs = dict(batch_size=batch_size, shuffle=bool(is_train), num_workers=int(num_workers), pin_memory=bool(pin_memory), drop_last=bool(is_train))
...
def build_deterministic_meta_loader(data_loader: DataLoader) -> DataLoader:
    ...
    kwargs: Dict[str, Any] = {'batch_size': batch_size, 'shuffle': False, 'num_workers': num_workers, 'pin_memory': pin_memory, 'drop_last': False}
...
class CharBERTTextEncoder(nn.Module):
    ...
    def __init__(self, feature_dim: int=256, seq_len: int=784, hidden_size: int=128, num_layers: int=2, num_heads: int=4, dropout: float=0.3, charbert_mode: str='charaware', char_vocab: str='hex', char_emb_dim: int=32, char_cnn_channels: int=64, char_fusion: str='gated', char_fusion_layers: str='all'):
        super().__init__()
        ...
        temporal_packet_layer = nn.TransformerEncoderLayer(d_model=self.char_hidden_size, nhead=num_heads, dim_feedforward=self.char_hidden_size * 4, dropout=dropout, batch_first=True)
    ...
    def _encode_temporal_session_summary(self, encoded: torch.Tensor, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor]=None) -> Optional[torch.Tensor]:
        if encoded.ndim != 3 or input_ids.ndim != 2 or encoded.size(0) != input_ids.size(0):
            raise ValueError('encoded/input_ids batch dimensions do not match')
        summaries: list[torch.Tensor] = []
        for batch_idx in range(encoded.size(0)):
            ...
            for (packet_idx, record) in enumerate(packet_records):
                ...
                packet_vec = packet_vec + self.temporal_packet_pos_emb(torch.tensor([pos_idx], device=encoded.device, dtype=torch.long))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if enc is not None:
            ...
            cls_summary = enc[:, 0]
            packet_summary = self._encode_temporal_session_summary(enc, x, attention_mask=attention_mask)
...
class AttentionFusionModel(nn.Module):
    ...
    def __init__(self, num_classes: int=10, attention_dim: int=256, char_hidden_size: int=128, charbert_mode: str='charaware', char_vocab: str='hex', char_emb_dim: int=32, char_cnn_channels: int=64, char_fusion: str='gated', char_fusion_layers: str='all', fusion_mode: str='attention'):
        super().__init__()
        ...
        self.text_encoder = CharBERTTextEncoder(feature_dim=char_hidden_size, seq_len=784, hidden_size=char_hidden_size, num_layers=2, num_heads=4, dropout=0.3, charbert_mode=charbert_mode, char_vocab=char_vocab, char_emb_dim=char_emb_dim, char_cnn_channels=char_cnn_channels, char_fusion=char_fusion, char_fusion_layers=char_fusion_layers)
        ...
        if self.fusion_mode == 'attention':
            ...
            self.pcap_linear = nn.Linear(1, attention_dim)
            fusion_out_dim = mobilevit_feature_dim + attention_dim
        self.out = nn.Sequential(nn.Linear(fusion_out_dim, 512), nn.ReLU(), nn.Dropout(0.3), nn.Linear(512, num_classes))
...
def initialize_fusion_model(num_classes: int, fusion_mode: str='attention', attention_dim: int=256, charbert_mode: str='charaware', char_vocab: str='hex', char_emb_dim: int=32, char_cnn_channels: int=64, char_fusion: str='gated', char_fusion_layers: str='all') -> nn.Module:
    ...
    model = AttentionFusionModel(num_classes=num_classes, attention_dim=attention_dim, charbert_mode=charbert_mode, char_vocab=char_vocab, char_emb_dim=char_emb_dim, char_cnn_channels=char_cnn_channels, char_fusion=char_fusion, char_fusion_layers=char_fusion_layers, fusion_mode=fusion_mode)
    logger.info('融合模型初始化完成，分类头设置为 %s 个类别', num_classes)
    return model

def evaluate_epoch(model: nn.Module, data_loader: DataLoader, criterion: nn.Module, device: torch.device, *, use_amp: bool=False):
    model.eval()
    val_loss = 0.0
    val_total = 0
    val_corrects = 0
...
def train_fusion_model(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, num_epochs: int, learning_rate: float, device: torch.device, patience: int=4, use_amp: bool=True, class_balance: str='none', loss_type: str='ce', focal_gamma: float=2.0, weight_decay: float=0.0001, label_smoothing: float=0.0, early_stop_metric: str='val_loss', early_stop_mode: str='auto', lr_scheduler_mode: str='reduce', lr_patience: int=2, lr_factor: float=0.5, min_lr: float=1e-06, grad_clip_norm: float=1.0, val_every: int=1, max_consecutive_invalid_batches: int=128):
    logger.info('开始训练融合模型')
    logger.info('训练参数 - Epochs: %s, Learning Rate: %s, Early Stopping Patience: %s', num_epochs, learning_rate, patience)
    logger.info('训练策略 - class_balance: %s, loss_type: %s, focal_gamma: %.3f, weight_decay: %.6f, label_smoothing: %.4f, early_stop_metric: %s, lr_scheduler: %s, val_every: %s', class_balance, loss_type, focal_gamma, weight_decay, label_smoothing, early_stop_metric, lr_scheduler_mode, val_every)
    logger.info('训练保护 - grad_clip_norm: %.3f, max_consecutive_invalid_batches: %s', grad_clip_norm, max_consecutive_invalid_batches)
    logger.info('DataLoader参数 - train_batches: %s, val_batches: %s, batch_size: %s, num_workers: %s, pin_memory: %s', len(train_loader), len(val_loader), getattr(train_loader, 'batch_size', 'unknown'), getattr(train_loader, 'num_workers', 'unknown'), getattr(train_loader, 'pin_memory', 'unknown'))
    model.to(device)
    use_amp = bool(use_amp and device.type == 'cuda')
    non_blocking = bool(device.type == 'cuda')
    class_weights = None
    ...
    if loss_type == 'focal':
        criterion = FocalCrossEntropyLoss(gamma=float(focal_gamma), weight=class_weights, label_smoothing=float(max(label_smoothing, 0.0)))
    else:
    ...
    if lr_scheduler_mode == 'reduce':
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode=mode, factor=float(lr_factor), patience=int(lr_patience), min_lr=float(min_lr))
    elif lr_scheduler_mode == 'cosine':
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max(int(num_epochs), 1), eta_min=float(min_lr))
    history = {'train_loss': [], 'train_acc': [], 'train_f1': [], 'val_loss': [], 'val_acc': [], 'val_f1': []}
    health = {'run_status': 'ok', 'stop_reason': 'completed', 'invalid_loss_batches': 0, 'invalid_grad_batches': 0, 'invalid_param_events': 0, 'processed_train_batches': 0, 'skipped_train_batches': 0}
    ...
    for epoch in range(num_epochs):
        ...
        for (batch_idx, (images, pcap_data, labels)) in enumerate(train_progress):
            ...
            if not torch.isfinite(loss):
                ...
                if _should_log_invalid_batch(health['invalid_loss_batches']):
                    logger.warning('训练损失无效（NaN/Inf），跳过该 batch: epoch=%s batch=%s/%s', epoch + 1, batch_idx + 1, len(train_loader))
                if consecutive_invalid_batches >= max_consecutive_invalid_batches:
                    ...
                    logger.error('连续无效 batch 达到阈值，终止训练: epoch=%s batch=%s/%s reason=%s', epoch + 1, batch_idx + 1, len(train_loader), invalid_stop_reason)
            ...
            if use_amp:
                ...
                if _has_non_finite_gradients(model):
                    ...
                    if _should_log_invalid_batch(health['invalid_grad_batches']):
                        logger.warning('梯度无效（NaN/Inf），跳过该 batch: epoch=%s batch=%s/%s', epoch + 1, batch_idx + 1, len(train_loader))
                    if consecutive_invalid_batches >= max_consecutive_invalid_batches:
            ...
            if _has_non_finite_parameters(model):
                ...
                logger.error('检测到模型参数出现 NaN/Inf，终止训练: epoch=%s batch=%s/%s', epoch + 1, batch_idx + 1, len(train_loader))
                stop_training_now = True
                break
        ...
        run_validation = not stop_training_now and ((epoch + 1) % max(int(val_every), 1) == 0 or epoch + 1 == num_epochs)
        if run_validation:
        ...
        if stop_training_now:
            ...
            break
        if run_validation:
            logger.info('Epoch %s 结果: 训练 Loss: %.4f, 训练 Acc: %.4f, 训练 F1: %.4f | 验证 Loss: %.4f, 验证 Acc: %.4f, 验证 F1: %.4f', epoch + 1, epoch_train_loss, epoch_train_acc, epoch_train_f1, val_loss, val_acc, val_f1)
            ...
            if monitor_is_finite:
                ...
                logger.warning('跳过本轮早停更新：%s=%.6f 不是有限值（可能是 NaN/Inf）', early_stop_metric, monitor_value)
            ...
            logger.info('Epoch %s 结果: 训练 Loss: %.4f, 训练 Acc: %.4f, 训练 F1: %.4f | 跳过验证 (val_every=%s)', epoch + 1, epoch_train_loss, epoch_train_acc, epoch_train_f1, val_every)
...
def evaluate_full(model: nn.Module, data_loader: DataLoader, device: torch.device):
    ...
    (loss, acc, macro_f1, labels, preds) = evaluate_epoch(model, data_loader, criterion, device, use_amp=device.type == 'cuda')
    ...
    return dict(loss=loss, acc=acc, macro_f1=macro_f1, report=report, cm=cm, per_class_f1=per_class_f1, labels=labels, preds=preds)
...
def save_report_md(path: Path, *, title: str, acc: float, macro_f1: float, report: str, cm: np.ndarray, confusion_image: str, curve_image: str) -> None:
...
def summarize_attention(attn: np.ndarray, pad_mask: Optional[np.ndarray]=None) -> dict:
    ...
    out = dict(mean=mean, max=mx, min=mn, entropy=ent_mean, top1=topk_mass(1), top5=topk_mass(5), top10=topk_mass(10))
...
def collect_attention_diagnostics(model: nn.Module, data_loader: DataLoader, device: torch.device, output_dir: Path, *, prefix: str, logger_obj, max_batches: int=6, filename: Optional[str]=None) -> Optional[Path]:
    try:
        model.eval()
        attn_collect = []
        pad_collect = []
        ...
        logger_obj.info('[AttentionDiag] mean=%.6f min=%.6f max=%.6f entropy=%.4f top1=%.4f top5=%.4f top10=%.4f nonpad_mass=%.4f pad_mass=%.4f nonpad_frac=%.4f', stats.get('mean', float('nan')), stats.get('min', float('nan')), stats.get('max', float('nan')), stats.get('entropy', float('nan')), stats.get('top1', float('nan')), stats.get('top5', float('nan')), stats.get('top10', float('nan')), stats.get('nonpad_mass_mean', float('nan')), stats.get('pad_mass_mean', float('nan')), stats.get('nonpad_fraction_mean', float('nan')))
        eps = 1e-12
        nonpad = ~pad_all
...
def build_meta_features_from_probs(text_probs: np.ndarray, image_probs: np.ndarray, *, fusion_probs: Optional[np.ndarray]=None) -> np.ndarray:
    ...
    agreement = np.stack([(text_pred == image_pred).astype(np.float64), (text_pred == fusion_pred).astype(np.float64), (image_pred == fusion_pred).astype(np.float64)], axis=1)
    meta_features = np.concatenate([text_probs, image_probs, fusion_probs, _entropy_features(text_probs), _entropy_features(image_probs), _entropy_features(fusion_probs), _margin_features(text_probs), _margin_features(image_probs), _margin_features(fusion_probs), agreement], axis=1)
...
def build_inverse_frequency_sample_weights(labels: np.ndarray) -> np.ndarray:
    ...
    class_weights = {int(c): float(labels.size) / float(max(1, len(classes)) * max(1, cnt)) for (c, cnt) in zip(classes.tolist(), counts.tolist())}
...
def apply_class_gains(probs: np.ndarray, class_gains: Dict[int, float]) -> np.ndarray:
    ...
    for (cls, gain) in class_gains.items():
        if 0 <= int(cls) < tuned.shape[1]:
            tuned[:, int(cls)] *= float(max(gain, 1e-06))
    return _normalize_probs(tuned)

def tune_class_gains(*, labels: np.ndarray, probs: np.ndarray, target_classes: List[int], gain_grid: Optional[List[float]]=None) -> Dict[int, float]:
...
def fit_binary_centroid_head(features: np.ndarray, labels: np.ndarray, *, class_a: int, class_b: int) -> Optional[Dict[str, Any]]:
    ...
    var = np.var(pair, axis=0) + 1e-06
    return {'class_a': int(class_a), 'class_b': int(class_b), 'centroid_a': np.mean(xa, axis=0), 'centroid_b': np.mean(xb, axis=0), 'var': var}

def apply_binary_correction_for_pair(*, preds: np.ndarray, probs: np.ndarray, features: np.ndarray, head: Optional[Dict[str, Any]], class_a: int, class_b: int, alpha: float=1.0) -> Tuple[np.ndarray, np.ndarray]:
    ...
    pair_mass = np.clip(corrected_probs[mask, ca] + corrected_probs[mask, cb], 1e-06, 1.0)
    ...
    corrected_preds[mask] = np.where(pa_corr > pb_corr, ca, np.where(pa_corr < pb_corr, cb, corrected_preds[mask]))
    return (corrected_preds, corrected_probs)

def tune_binary_correction_alpha_for_pair(*, labels: np.ndarray, probs: np.ndarray, features: np.ndarray, head: Optional[Dict[str, Any]], class_a: int, class_b: int, objective: str='macro_f1', alpha_grid: Optional[List[float]]=None) -> float:
    if head is None:
        return 0.0
    labels = np.asarray(labels, dtype=np.int64).reshape(-1)
    probs = _normalize_probs(probs)
    ...
    for alpha in alpha_grid:
        (preds, _) = apply_binary_correction_for_pair(preds=base_preds, probs=probs, features=features, head=head, class_a=class_a, class_b=class_b, alpha=alpha)
        if objective == 'accuracy':
...
def apply_pair_temperature(*, probs: np.ndarray, class_a: int, class_b: int, temperature: float) -> np.ndarray:
    calibrated = _normalize_probs(probs).copy()
    ca = int(class_a)
    cb = int(class_b)
    if ca < 0 or cb < 0 or ca >= calibrated.shape[1] or (cb >= calibrated.shape[1]):
        return calibrated
    t = float(max(temperature, 1e-06))
...
def tune_pair_temperature(*, labels: np.ndarray, probs: np.ndarray, class_a: int, class_b: int, temperature_grid: Optional[List[float]]=None) -> float:
    labels = np.asarray(labels, dtype=np.int64).reshape(-1)
    probs = _normalize_probs(probs)
...
def apply_pair_threshold(*, preds: np.ndarray, probs: np.ndarray, class_a: int, class_b: int, threshold: float) -> np.ndarray:
...
def tune_pair_threshold(*, labels: np.ndarray, probs: np.ndarray, class_a: int, class_b: int, objective: str='pair_f1', threshold_grid: Optional[List[float]]=None) -> float:
    labels = np.asarray(labels, dtype=np.int64).reshape(-1)
    probs = _normalize_probs(probs)
    if labels.size == 0 or probs.shape[0] != labels.size:
        return 0.5
...
def calibrate_multiclass_probs(*, labels: np.ndarray, oof_probs: np.ndarray, test_probs: np.ndarray, calibration: str) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    y = np.asarray(labels, dtype=np.int64).reshape(-1)
...
def apply_per_class_thresholds(*, probs: np.ndarray, thresholds: np.ndarray) -> np.ndarray:
    ...
    adjusted = p / np.clip(tau, 1e-06, None)
    return np.argmax(adjusted, axis=1).astype(np.int64)

def tune_per_class_thresholds(*, labels: np.ndarray, probs: np.ndarray, minority_classes: List[int], objective: str, minority_lambda: float, grid: Optional[List[float]]=None) -> np.ndarray:
    y = np.asarray(labels, dtype=np.int64).reshape(-1)
    p = _normalize_probs(probs)
    if y.size == 0 or p.shape[0] != y.size:
...
def compute_threshold_objective_value(*, labels: np.ndarray, preds: np.ndarray, minority_classes: List[int], objective: str, minority_lambda: float) -> float:
    y = np.asarray(labels, dtype=np.int64).reshape(-1)
...
def build_single_layer_baseline_result(method_results: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    return {'method': 'single_layer_baseline', 'source_method': str(best.get('method', '')), 'acc': float(best.get('acc', 0.0)), 'macro_f1': float(best.get('macro_f1', 0.0)), 'oof_acc': float(best.get('oof_acc', 0.0)), 'oof_macro_f1': float(best.get('oof_macro_f1', 0.0)), 'report': best.get('report', ''), 'confusion_matrix': best.get('confusion_matrix', np.zeros((0, 0), dtype=int)), 'confusion_matrix_path': best.get('confusion_matrix_path'), 'report_path': best.get('report_path'), 'meta_model_path': best.get('meta_model_path'), 'postprocess': {'type': 'single_layer_best_by_oof'}}
...
def run_level2_blender_oof(*, labels: np.ndarray, level1_oof_probs: Dict[str, np.ndarray], level1_test_probs: Dict[str, np.ndarray], oof_folds: int, minority_classes: List[int], threshold_objective: str, minority_lambda: float) -> Dict[str, Any]:
    y = np.asarray(labels, dtype=np.int64).reshape(-1)
    ...
    if y.size == 0 or x_oof.shape[0] != y.size or x_test.shape[0] == 0:
        return {'preds': np.array([], dtype=np.int64), 'probs': np.zeros((0, 0), dtype=np.float64), 'oof_probs': np.zeros((0, 0), dtype=np.float64), 'thresholds': np.array([], dtype=np.float64)}
    ...
    def _fit_predict(train_x: np.ndarray, train_y: np.ndarray, valid_x: np.ndarray) -> np.ndarray:
        ...
        return fold_probs
    oof_probs = compute_oof_predictions(features=x_oof, labels=y, n_splits=max(2, int(oof_folds)), seed=42, fit_predict_fn=_fit_predict)
    ...
    thresholds = tune_per_class_thresholds(labels=y, probs=oof_probs, minority_classes=minority_classes, objective=threshold_objective, minority_lambda=minority_lambda)
    ...
    threshold_objective_value = compute_threshold_objective_value(labels=y, preds=oof_threshold_preds, minority_classes=minority_classes, objective=threshold_objective, minority_lambda=minority_lambda)
    return {'preds': preds, 'probs': test_probs, 'oof_probs': oof_probs, 'thresholds': thresholds, 'threshold_objective_value': float(threshold_objective_value), 'blender': blender}

def build_two_level_postprocess_payload(*, stacking_level: str, calibration: Dict[str, float], thresholds: np.ndarray, minority_classes: List[int], minority_recall_before: float, minority_recall_after: float, threshold_objective: str, objective_value: float, oof_test_gap: float) -> Dict[str, Any]:
    return {'stacking_level': str(stacking_level), 'calibration': dict(calibration), 'thresholds': [float(v) for v in np.asarray(thresholds, dtype=np.float64).tolist()], 'threshold_objective': str(threshold_objective), 'objective_value': float(objective_value), 'oof_test_gap': float(oof_test_gap), 'minority_metrics': {'classes': [int(c) for c in minority_classes], 'recall_before': float(minority_recall_before), 'recall_after': float(minority_recall_after)}}

def compute_oof_predictions(*, features: np.ndarray, labels: np.ndarray, n_splits: int, seed: int, fit_predict_fn: Callable[[np.ndarray, np.ndarray, np.ndarray], np.ndarray], on_fold: Optional[Callable[[int, np.ndarray, np.ndarray], None]]=None) -> np.ndarray:
...
def generate_meta_features(text_model: nn.Module, mobilevit_model: nn.Module, data_loader: DataLoader, device: torch.device, *, fusion_model: Optional[nn.Module]=None, use_softmax: bool=True):
    text_model.eval()
    ...
    with torch.no_grad():
        for (images, pcap_data, labels) in tqdm(data_loader, desc='生成元特征'):
            ...
            img_out = torch.softmax(mobilevit_logits, dim=1).cpu().numpy() if use_softmax else mobilevit_logits.cpu().numpy()
            fusion_out = None
            if fusion_model is not None:
                fusion_logits = fusion_model(images, pcap_data)
                if isinstance(fusion_logits, (tuple, list)):
                    fusion_logits = fusion_logits[0]
                fusion_out = torch.softmax(fusion_logits, dim=1).cpu().numpy() if use_softmax else fusion_logits.cpu().numpy()
...
def train_xgboost(meta_features: np.ndarray, meta_labels: np.ndarray, *, sample_weight: Optional[np.ndarray]=None):
    try:
        import xgboost as xgb
    except ImportError as e:
        raise ImportError('xgboost 未安装') from e
    n_classes = int(len(np.unique(meta_labels)))
    clf = xgb.XGBClassifier(n_estimators=350, max_depth=6, learning_rate=0.05, subsample=0.9, colsample_bytree=0.85, min_child_weight=2.0, reg_lambda=1.0, objective='multi:softprob', num_class=max(2, n_classes), eval_metric='mlogloss', random_state=42)
    fit_kwargs = {}
...
def train_meta_learner(meta_features: np.ndarray, meta_labels: np.ndarray, method: str='xgboost', *, sample_weight: Optional[np.ndarray]=None):
    if method == 'xgboost':
...
def build_common_kwargs(args):
    ...
    if getattr(args, 'task_name', ''):
        (train_image_dir, train_pcap_dir, test_image_dir, test_pcap_dir, resolved_dataset_name) = resolve_task_dataset_dirs(args.dataset_root, args.task_name, image_mode=getattr(args, 'image_mode', 'rgb'))
    else:
        (train_image_dir, train_pcap_dir, test_image_dir, test_pcap_dir, resolved_dataset_name) = resolve_dataset_dirs(args.dataset_root, args.dataset_name or None)
    ...
    return dict(train_image_dir=train_image_dir, train_pcap_dir=train_pcap_dir, test_image_dir=test_image_dir, test_pcap_dir=test_pcap_dir, batch_size=args.batch_size, image_size=args.image_size, max_pcap_length=args.max_pcap_length, epochs=args.epochs, lr=args.lr, patience=args.patience, device=device, output_dir=Path(args.output_dir), num_workers=args.num_workers, pin_memory=args.pin_memory, persistent_workers=args.persistent_workers, prefetch_factor=args.prefetch_factor, attention_dim=args.attention_dim, fusion_mode=args.fusion_mode, use_temporal_sidecar=not args.no_temporal, charbert_mode=args.charbert_mode, char_vocab=args.char_vocab, char_emb_dim=args.char_emb_dim, char_cnn_channels=args.char_cnn_channels, char_fusion=args.char_fusion, char_fusion_layers=args.char_fusion_layers, stacking_level=args.stacking_level, stacking_calibration=args.stacking_calibration, stacking_threshold_objective=args.stacking_threshold_objective, stacking_minority_lambda=args.stacking_minority_lambda, stacking_oof_folds=args.stacking_oof_folds, use_amp=not args.no_amp, use_index_cache=not args.no_index_cache, rebuild_index_cache=args.rebuild_index_cache, class_balance=args.class_balance, loss_type=args.loss_type, focal_gamma=args.focal_gamma, weight_decay=args.weight_decay, label_smoothing=args.label_smoothing, early_stop_metric=args.early_stop_metric, early_stop_mode=args.early_stop_mode, lr_scheduler_mode=args.lr_scheduler, lr_patience=args.lr_patience, lr_factor=args.lr_factor, min_lr=args.min_lr, grad_clip_norm=args.grad_clip_norm, val_every=args.val_every, max_consecutive_invalid_batches=args.max_consecutive_invalid_batches, selected_groups=parse_csv_values(args.cic_group) if args.cic_group else None, image_mode=getattr(args, 'image_mode', 'rgb'))
...
def run_fusion_experiment(*, fusion_mode: str, train_image_dir: str, train_pcap_dir: str, test_image_dir: str, test_pcap_dir: str, batch_size: int, image_size: int, max_pcap_length: int, epochs: int, lr: float, patience: int, device: torch.device, output_dir: Path, num_workers: int, pin_memory: bool, persistent_workers: bool, prefetch_factor: int, attention_dim: int=256, use_temporal_sidecar: bool=True, charbert_mode: str='charaware', char_vocab: str='hex', char_emb_dim: int=32, char_cnn_channels: int=64, char_fusion: str='gated', char_fusion_layers: str='all', use_amp: bool=True, use_index_cache: bool=True, rebuild_index_cache: bool=False, class_balance: str='none', loss_type: str='ce', focal_gamma: float=2.0, weight_decay: float=0.0001, label_smoothing: float=0.0, early_stop_metric: str='val_loss', early_stop_mode: str='auto', lr_scheduler_mode: str='reduce', lr_patience: int=2, lr_factor: float=0.5, min_lr: float=1e-06, grad_clip_norm: float=1.0, val_every: int=1, max_consecutive_invalid_batches: int=128, selected_groups: Optional[List[str]]=None, image_mode: str='rgb') -> None:
    ensure_output_dirs(output_dir)
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    (train_loader, train_classes) = load_fusion_data(train_image_dir, train_pcap_dir, batch_size, image_size, max_pcap_length, num_workers=num_workers, pin_memory=pin_memory, persistent_workers=persistent_workers, prefetch_factor=prefetch_factor, use_index_cache=use_index_cache, rebuild_index_cache=rebuild_index_cache, is_train=True, balance_mode=class_balance, selected_groups=selected_groups, use_temporal_sidecar=use_temporal_sidecar, image_mode=image_mode)
    (test_loader, test_classes) = load_fusion_data(test_image_dir, test_pcap_dir, batch_size, image_size, max_pcap_length, num_workers=num_workers, pin_memory=pin_memory, persistent_workers=persistent_workers, prefetch_factor=prefetch_factor, use_index_cache=use_index_cache, rebuild_index_cache=rebuild_index_cache, is_train=False, balance_mode='none', selected_groups=selected_groups, use_temporal_sidecar=use_temporal_sidecar, image_mode=image_mode)
    assert train_classes == test_classes, '训练集和测试集类别不一致'
    num_classes = len(train_classes)
    model = initialize_fusion_model(num_classes, fusion_mode, attention_dim=attention_dim, charbert_mode=charbert_mode, char_vocab=char_vocab, char_emb_dim=char_emb_dim, char_cnn_channels=char_cnn_channels, char_fusion=char_fusion, char_fusion_layers=char_fusion_layers)
    (model, history) = train_fusion_model(model, train_loader, test_loader, epochs, lr, device, patience, use_amp=use_amp, class_balance=class_balance, loss_type=loss_type, focal_gamma=focal_gamma, weight_decay=weight_decay, label_smoothing=label_smoothing, early_stop_metric=early_stop_metric, early_stop_mode=early_stop_mode, lr_scheduler_mode=lr_scheduler_mode, lr_patience=lr_patience, lr_factor=lr_factor, min_lr=min_lr, grad_clip_norm=grad_clip_norm, val_every=val_every, max_consecutive_invalid_batches=max_consecutive_invalid_batches)
    attention_curve_path: Optional[Path] = None
    if fusion_mode == 'attention':
        attention_curve_path = collect_attention_diagnostics(model, test_loader, device, run_dir, prefix=f'{tag}_{ts}', logger_obj=run_logger, filename=artifact_paths['attention_curve'].name)
    ...
    save_report_md(report_path, title=f'融合方式: {fusion_mode}', acc=eval_result['acc'], macro_f1=eval_result['macro_f1'], report=eval_result['report'], cm=eval_result['cm'], confusion_image=cm_path.name, curve_image=curve_path.name)
    log_saved(run_logger, report_path, 'report')
    model_path = artifact_paths['model']
    torch.save(model.state_dict(), model_path)
    log_saved(run_logger, model_path, 'model')
    metrics_payload = {'mode': fusion_mode, 'tag': tag, 'timestamp': ts, 'run_name': run_dir.name, 'run_dir': str(run_dir), 'output_root': str(output_dir), 'classes': train_classes, 'history': history, 'run_status': history.get('health', {}).get('run_status', 'ok'), 'stop_reason': history.get('health', {}).get('stop_reason', 'completed'), 'health': history.get('health', {}), 'eval': {'loss': eval_result['loss'], 'acc': eval_result['acc'], 'macro_f1': eval_result['macro_f1'], 'report': eval_result['report'], 'confusion_matrix': eval_result['cm'], 'per_class_f1': eval_result['per_class_f1']}, 'artifacts': {'train_log': log_path.name, 'metrics_curve': curve_path.name, 'confusion_matrix': cm_path.name, 'attention_curve': attention_curve_path.name if attention_curve_path else None, 'report': report_path.name, 'model': model_path.name}}
    (metrics_path, epoch_csv_path) = export_metrics_artifacts(run_dir=run_dir, history=history, metrics_payload=metrics_payload)
...
def run_stacking_experiment(*, base_fusion_mode: str, meta_methods: Iterable[str], train_image_dir: str, train_pcap_dir: str, test_image_dir: str, test_pcap_dir: str, batch_size: int, image_size: int, max_pcap_length: int, epochs: int, lr: float, patience: int, device: torch.device, output_dir: Path, num_workers: int, pin_memory: bool, persistent_workers: bool, prefetch_factor: int, attention_dim: int=256, use_temporal_sidecar: bool=True, charbert_mode: str='charaware', char_vocab: str='hex', char_emb_dim: int=32, char_cnn_channels: int=64, char_fusion: str='gated', char_fusion_layers: str='all', stacking_level: str='two_level', stacking_calibration: str='temp', stacking_threshold_objective: str='macro_f1_minority_recall', stacking_minority_lambda: float=0.3, stacking_oof_folds: int=5, ensemble_tag: Optional[str]=None, use_amp: bool=True, use_index_cache: bool=True, rebuild_index_cache: bool=False, class_balance: str='none', loss_type: str='ce', focal_gamma: float=2.0, weight_decay: float=0.0001, label_smoothing: float=0.0, early_stop_metric: str='val_loss', early_stop_mode: str='auto', lr_scheduler_mode: str='reduce', lr_patience: int=2, lr_factor: float=0.5, min_lr: float=1e-06, grad_clip_norm: float=1.0, val_every: int=1, max_consecutive_invalid_batches: int=128, selected_groups: Optional[List[str]]=None, image_mode: str='rgb') -> None:
    ensure_output_dirs(output_dir)
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    run_logger.info('stacking config: level=%s calibration=%s threshold_objective=%s minority_lambda=%.3f oof_folds=%d', stacking_level, stacking_calibration, stacking_threshold_objective, float(stacking_minority_lambda), int(stacking_oof_folds))
    methods = list(meta_methods)
    (train_loader, train_classes) = load_fusion_data(train_image_dir, train_pcap_dir, batch_size, image_size, max_pcap_length, num_workers=num_workers, pin_memory=pin_memory, persistent_workers=persistent_workers, prefetch_factor=prefetch_factor, use_index_cache=use_index_cache, rebuild_index_cache=rebuild_index_cache, is_train=True, balance_mode=class_balance, selected_groups=selected_groups, use_temporal_sidecar=use_temporal_sidecar, image_mode=image_mode)
    (test_loader, test_classes) = load_fusion_data(test_image_dir, test_pcap_dir, batch_size, image_size, max_pcap_length, num_workers=num_workers, pin_memory=pin_memory, persistent_workers=persistent_workers, prefetch_factor=prefetch_factor, use_index_cache=use_index_cache, rebuild_index_cache=rebuild_index_cache, is_train=False, balance_mode='none', selected_groups=selected_groups, use_temporal_sidecar=use_temporal_sidecar, image_mode=image_mode)
    assert train_classes == test_classes, '训练集和测试集类别不一致'
    num_classes = len(train_classes)
    model = initialize_fusion_model(num_classes, base_fusion_mode, attention_dim=attention_dim, charbert_mode=charbert_mode, char_vocab=char_vocab, char_emb_dim=char_emb_dim, char_cnn_channels=char_cnn_channels, char_fusion=char_fusion, char_fusion_layers=char_fusion_layers)
    (model, history) = train_fusion_model(model, train_loader, test_loader, epochs, lr, device, patience, use_amp=use_amp, class_balance=class_balance, loss_type=loss_type, focal_gamma=focal_gamma, weight_decay=weight_decay, label_smoothing=label_smoothing, early_stop_metric=early_stop_metric, early_stop_mode=early_stop_mode, lr_scheduler_mode=lr_scheduler_mode, lr_patience=lr_patience, lr_factor=lr_factor, min_lr=min_lr, grad_clip_norm=grad_clip_norm, val_every=val_every, max_consecutive_invalid_batches=max_consecutive_invalid_batches)
    attention_curve_path: Optional[Path] = None
    if base_fusion_mode == 'attention':
        attention_curve_path = collect_attention_diagnostics(model, test_loader, device, run_dir, prefix=f'{ensemble_tag}_{ts}', logger_obj=run_logger, filename=artifact_paths['attention_curve'].name)
        if attention_curve_path is not None:
            log_saved(run_logger, attention_curve_path, 'attention_curve')
    curve_path = artifact_paths['metrics_curve']
    ...
    log_saved(run_logger, cm_path, 'confusion_matrix')
    report_path = artifact_paths['report_md']
    save_report_md(report_path, title=f'融合方式: {base_fusion_mode}+stacking ({ensemble_tag})', acc=base_eval['acc'], macro_f1=base_eval['macro_f1'], report=base_eval['report'], cm=base_eval['cm'], confusion_image=cm_path.name, curve_image=curve_path.name)
    log_saved(run_logger, report_path, 'report')
    ...
    (meta_features, meta_labels) = generate_meta_features(model.text_encoder, model.mobilevit, meta_train_loader, device, fusion_model=model)
    (test_meta_features, test_meta_labels) = generate_meta_features(model.text_encoder, model.mobilevit, meta_test_loader, device, fusion_model=model)
    run_logger.info('meta feature extraction completed: train_samples=%s test_samples=%s', int(len(meta_labels)), int(len(test_meta_labels)))
    sample_weights = build_inverse_frequency_sample_weights(meta_labels)
    class_count = len(train_classes)
    (is_mta_task, is_mfcp_task) = detect_stacking_special_tasks(train_classes=train_classes, train_image_dir=train_image_dir, train_pcap_dir=train_pcap_dir)
    ...
    if is_mfcp_task and mfcp_preferred_pair is None and (mfcp_fallback_pair is None):
        run_logger.warning('MFCP pair correction skipped: no preferred/fallback pair found, classes=%s', train_classes)
    ...
    for method in methods:
        try:
            ...
            def _fit_predict(train_x: np.ndarray, train_y: np.ndarray, valid_x: np.ndarray) -> np.ndarray:
                ...
                fold_model = train_meta_learner(train_x, train_y, method=method, sample_weight=fold_weights)
                (_, fold_probs) = _predict_with_meta_model(fold_model, valid_x, num_classes=class_count)
                return fold_probs
            oof_probs = compute_oof_predictions(features=meta_features, labels=meta_labels, n_splits=max(2, int(stacking_oof_folds)), seed=42, fit_predict_fn=_fit_predict)
            meta_model = train_meta_learner(meta_features, meta_labels, method=method, sample_weight=sample_weights)
        except ImportError as e:
        ...
        (oof_probs, pred_probs, calibration_info) = calibrate_multiclass_probs(labels=meta_labels, oof_probs=oof_probs, test_probs=pred_probs, calibration=stacking_calibration)
        ...
        if is_mfcp_task and len(meta_labels):
            dynamic_pair = select_confusion_pair(labels=meta_labels, preds=np.argmax(oof_probs, axis=1).astype(np.int64), preferred_pair=mfcp_preferred_pair)
            ...
            if dynamic_pair is None:
                ...
                pair_alpha = tune_binary_correction_alpha_for_pair(labels=meta_labels, probs=oof_probs, features=meta_features, head=head, class_a=pair_class_a, class_b=pair_class_b, objective=pair_tuning_objective)
                oof_pair_probs = oof_probs
                if len(oof_probs):
                    (oof_pair_preds, oof_pair_probs) = apply_binary_correction_for_pair(preds=np.argmax(oof_probs, axis=1).astype(np.int64), probs=oof_probs, features=meta_features, head=head, class_a=pair_class_a, class_b=pair_class_b, alpha=pair_alpha)
                    pair_temperature = tune_pair_temperature(labels=meta_labels, probs=oof_pair_probs, class_a=pair_class_a, class_b=pair_class_b)
                    oof_pair_probs = apply_pair_temperature(probs=oof_pair_probs, class_a=pair_class_a, class_b=pair_class_b, temperature=pair_temperature)
                    pair_threshold = tune_pair_threshold(labels=meta_labels, probs=oof_pair_probs, class_a=pair_class_a, class_b=pair_class_b, objective=pair_tuning_objective)
                    oof_pair_preds = apply_pair_threshold(preds=oof_pair_preds, probs=oof_pair_probs, class_a=pair_class_a, class_b=pair_class_b, threshold=pair_threshold)
                    method_oof_probs_for_vote = oof_pair_probs
                else:
                    pair_temperature = 1.0
                    pair_threshold = 0.5
                (preds, pred_probs) = apply_binary_correction_for_pair(preds=preds, probs=pred_probs, features=test_meta_features, head=head, class_a=pair_class_a, class_b=pair_class_b, alpha=pair_alpha)
                pred_probs = apply_pair_temperature(probs=pred_probs, class_a=pair_class_a, class_b=pair_class_b, temperature=pair_temperature)
                preds = apply_pair_threshold(preds=np.argmax(pred_probs, axis=1).astype(np.int64), probs=pred_probs, class_a=pair_class_a, class_b=pair_class_b, threshold=pair_threshold)
        ...
        if oof_test_gap > 0.12:
            run_logger.warning('[%s] OOF-test gap is large: oof_macro_f1=%.4f test_macro_f1=%.4f gap=%.4f', tag, oof_macro_f1, macro_f1, oof_test_gap)
        if report:
        ...
        save_report_md(method_report_path, title=f'融合方式: {base_fusion_mode}+stacking ({method})', acc=acc, macro_f1=macro_f1, report=report, cm=cm, confusion_image=method_cm_path.name, curve_image=curve_path.name)
        ...
        method_results.append({'method': method, 'acc': acc, 'macro_f1': macro_f1, 'oof_acc': oof_acc, 'oof_macro_f1': oof_macro_f1, 'report': report, 'confusion_matrix': cm, 'confusion_matrix_path': method_cm_path.name, 'report_path': method_report_path.name, 'meta_model_path': meta_model_path.name if meta_model_path else None, 'postprocess': postprocess})
        ...
        successful_method_weights.append(float(max(oof_weight_macro_f1, 1e-06)))
    effective_level = resolve_effective_stacking_level(requested_level=stacking_level, successful_methods=successful_method_names)
    if effective_level == 'two_level':
        ...
        level2_out = run_level2_blender_oof(labels=meta_labels, level1_oof_probs=method_oof_probs_map, level1_test_probs=method_test_probs_map, oof_folds=stacking_oof_folds, minority_classes=minority_classes, threshold_objective=stacking_threshold_objective, minority_lambda=stacking_minority_lambda)
        ...
        if len(test_meta_labels) and two_level_probs.shape[0] == len(test_meta_labels):
            ...
            if minority_classes:
                before_minor = float(recall_score(test_meta_labels, before_preds, labels=minority_classes, average='macro', zero_division=0))
                after_minor = float(recall_score(test_meta_labels, two_level_preds, labels=minority_classes, average='macro', zero_division=0))
            else:
                before_minor = 0.0
                after_minor = 0.0
            ...
            if two_level_oof_test_gap > 0.12:
                run_logger.warning('[%s_two_level_blender] OOF-test gap is large: oof_macro_f1=%.4f test_macro_f1=%.4f gap=%.4f', ensemble_tag, float(oof_two_macro_f1), float(two_level_macro_f1), float(two_level_oof_test_gap))
            ...
            save_report_md(two_level_report_path, title=f'融合方式: {base_fusion_mode}+stacking (two_level_blender)', acc=two_level_acc, macro_f1=two_level_macro_f1, report=two_level_report, cm=two_level_cm, confusion_image=two_level_cm_path.name, curve_image=curve_path.name)
            ...
            postprocess = build_two_level_postprocess_payload(stacking_level='two_level', calibration={'method': str(stacking_calibration), 'ece': float(calibration_metrics.get('ece', 0.0)), 'brier': float(calibration_metrics.get('brier', 0.0))}, thresholds=two_level_thresholds, minority_classes=minority_classes, minority_recall_before=before_minor, minority_recall_after=after_minor, threshold_objective=stacking_threshold_objective, objective_value=two_level_objective_value, oof_test_gap=two_level_oof_test_gap)
            method_results.append({'method': 'two_level_blender', 'acc': float(two_level_acc), 'macro_f1': float(two_level_macro_f1), 'oof_acc': float(oof_two_acc), 'oof_macro_f1': float(oof_two_macro_f1), 'report': two_level_report, 'confusion_matrix': two_level_cm, 'confusion_matrix_path': two_level_cm_path.name, 'report_path': two_level_report_path.name, 'meta_model_path': None, 'postprocess': postprocess})
            run_logger.info('[%s_two_level_blender] acc=%.4f macro_f1=%.4f oof_macro_f1=%.4f', ensemble_tag, float(two_level_acc), float(two_level_macro_f1), float(oof_two_macro_f1))
        else:
    ...
    if len(successful_method_probs) >= 2:
        ...
        vote_postprocess: Dict[str, Any] = {'weights': [float(w) for w in successful_method_weights], 'members': [m for m in successful_method_names]}
        ...
        if is_mfcp_task and len(meta_labels):
            dynamic_pair = select_confusion_pair(labels=meta_labels, preds=np.argmax(oof_vote_probs, axis=1).astype(np.int64) if len(oof_vote_probs) else np.array([], dtype=np.int64), preferred_pair=mfcp_preferred_pair)
            if dynamic_pair is None:
                dynamic_pair = mfcp_fallback_pair
            if dynamic_pair is None:
                ...
                vote_pair_alpha = tune_binary_correction_alpha_for_pair(labels=meta_labels, probs=oof_vote_probs, features=meta_features, head=vote_head, class_a=pair_class_a, class_b=pair_class_b, objective=pair_tuning_objective) if len(oof_vote_probs) else 0.0
                if len(oof_vote_probs):
                    oof_vote_preds = np.argmax(oof_vote_probs, axis=1).astype(np.int64)
                    (oof_vote_preds, oof_vote_probs) = apply_binary_correction_for_pair(preds=oof_vote_preds, probs=oof_vote_probs, features=meta_features, head=vote_head, class_a=pair_class_a, class_b=pair_class_b, alpha=vote_pair_alpha)
                    vote_pair_temperature = tune_pair_temperature(labels=meta_labels, probs=oof_vote_probs, class_a=pair_class_a, class_b=pair_class_b)
                    oof_vote_probs = apply_pair_temperature(probs=oof_vote_probs, class_a=pair_class_a, class_b=pair_class_b, temperature=vote_pair_temperature)
                    vote_pair_threshold = tune_pair_threshold(labels=meta_labels, probs=oof_vote_probs, class_a=pair_class_a, class_b=pair_class_b, objective=pair_tuning_objective)
                    oof_vote_preds = apply_pair_threshold(preds=oof_vote_preds, probs=oof_vote_probs, class_a=pair_class_a, class_b=pair_class_b, threshold=vote_pair_threshold)
                else:
                    vote_pair_temperature = 1.0
                    vote_pair_threshold = 0.5
                (vote_preds, vote_probs) = apply_binary_correction_for_pair(preds=vote_preds, probs=vote_probs, features=test_meta_features, head=vote_head, class_a=pair_class_a, class_b=pair_class_b, alpha=vote_pair_alpha)
                vote_probs = apply_pair_temperature(probs=vote_probs, class_a=pair_class_a, class_b=pair_class_b, temperature=vote_pair_temperature)
                vote_preds = apply_pair_threshold(preds=np.argmax(vote_probs, axis=1).astype(np.int64), probs=vote_probs, class_a=pair_class_a, class_b=pair_class_b, threshold=vote_pair_threshold)
        ...
        save_report_md(vote_report_path, title=f'融合方式: {base_fusion_mode}+stacking (soft_voting)', acc=vote_acc, macro_f1=vote_macro_f1, report=vote_report, cm=vote_cm, confusion_image=vote_cm_path.name, curve_image=curve_path.name)
        log_saved(run_logger, vote_report_path, 'report_soft_voting')
        method_results.append({'method': 'soft_voting', 'acc': vote_acc, 'macro_f1': vote_macro_f1, 'oof_acc': oof_vote_acc, 'oof_macro_f1': oof_vote_macro_f1, 'report': vote_report, 'confusion_matrix': vote_cm, 'confusion_matrix_path': vote_cm_path.name, 'report_path': vote_report_path.name, 'meta_model_path': None, 'postprocess': vote_postprocess})
    base_model_path = artifact_paths['base_model']
    torch.save(model.state_dict(), base_model_path)
    log_saved(run_logger, base_model_path, 'model_base')
    metrics_payload = {'mode': 'attention_stacking', 'base_fusion_mode': base_fusion_mode, 'tag': ensemble_tag, 'timestamp': ts, 'run_name': run_dir.name, 'run_dir': str(run_dir), 'output_root': str(output_dir), 'classes': train_classes, 'history': history, 'run_status': history.get('health', {}).get('run_status', 'ok'), 'stop_reason': history.get('health', {}).get('stop_reason', 'completed'), 'health': history.get('health', {}), 'base_eval': {'loss': base_eval['loss'], 'acc': base_eval['acc'], 'macro_f1': base_eval['macro_f1'], 'report': base_eval['report'], 'confusion_matrix': base_eval['cm'], 'per_class_f1': base_eval['per_class_f1']}, 'stacking': {'requested_level': stacking_level, 'effective_level': effective_level, 'calibration': stacking_calibration, 'threshold_objective': stacking_threshold_objective, 'minority_lambda': float(stacking_minority_lambda), 'oof_folds': int(stacking_oof_folds)}, 'meta_methods': methods, 'method_results': method_results, 'artifacts': {'train_log': log_path.name, 'metrics_curve': curve_path.name, 'confusion_matrix': cm_path.name, 'attention_curve': attention_curve_path.name if attention_curve_path else None, 'report': report_path.name, 'base_model': base_model_path.name}}
    (metrics_path, epoch_csv_path) = export_metrics_artifacts(run_dir=run_dir, history=history, metrics_payload=metrics_payload)
    log_saved(run_logger, metrics_path, 'metrics_json')
    log_saved(run_logger, epoch_csv_path, 'epoch_metrics_csv')
    try:
