...
def find_related_image_files(processed_root: Path, split: str, label: str, stem: str) -> List[Path]:
    image_dir = processed_root / 'image_data' / split / label
    found: List[Path] = []
    if not image_dir.exists():
        return found
    # Find files that start with the stem
...
def rebalance_processed(
    ...
    target_max = max(1, int(min_count * float(max_class_ratio)))
    logging.info('Found labels=%s; counts=%s; min_count=%s; target_max=%s', len(labels), counts, min_count, target_max)
    ...
    for label in labels:
        ...
        for src_path, split in selected:
            ...
            stem = src_path.stem
            related_images = find_related_image_files(processed_root, split, label, stem)
            ...
            manifest_rows.append({
                ...
                'session_name': meta.get('session_name', src_path.stem),
...
def build_parser():
    ...
    parser.add_argument('--max_class_ratio', type=float, default=5.0)
...
def main():
    ...
    rebalance_processed(processed_root=processed_root, dest_root=dest_root, max_class_ratio=args.max_class_ratio, seed=args.seed, copy=args.copy, force=args.force)
