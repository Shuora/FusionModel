...
def collect_image_index(processed_root: Path) -> Dict[str, Dict[str, Dict[str, List[Path]]]]:
    image_index: DefaultDict[str, DefaultDict[str, DefaultDict[str, List[Path]]]] = defaultdict(lambda : defaultdict(lambda : defaultdict(list)))
    for split in ('Train', 'Test'):
    ...
    return {split: {label: dict(stem_map) for (label, stem_map) in label_map.items()} for (split, label_map) in image_index.items()}
...
def find_related_image_files(image_index: Dict[str, Dict[str, Dict[str, List[Path]]]], split: str, label: str, stem: str) -> List[Path]:
    return list(image_index.get(split, {}).get(label, {}).get(stem, []))

def rebalance_processed(processed_root: Path, dest_root: Path, max_class_ratio: float, min_class_count: int, seed: int, copy: bool=False, force: bool=False) -> Path:
    ...
    logging.info('Found labels=%s; counts=%s; min_count=%s; effective_min=%s; target_max=%s', len(labels), counts, min_count, effective_min_count, target_max)
    manifest_rows = []
    total_linked = 0
    for label in labels:
        ...
        for (src_path, split) in selected:
            ...
            manifest_rows.append({'split': split, 'label': label, 'dataset_name': meta.get('dataset_name', ''), 'raw_path': meta.get('raw_path', str(src_path)), 'session_name': new_stem, 'bin_path': str(dst_bin)})
            total_linked += 1
...
def build_parser():
    ...
    parser.add_argument('--min_class_count', type=int, default=10000, help='Minimum samples per class; minority classes will be upsampled .')
...
def main():
    ...
    rebalance_processed(processed_root=processed_root, dest_root=dest_root, max_class_ratio=args.max_class_ratio, min_class_count=args.min_class_count, seed=args.seed, copy=args.copy, force=args.force)
