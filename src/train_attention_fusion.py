...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default="auto",
        help="USTC profile presets for faster convergence.",
    )
...
def main() -> None:
    ...
    report_text = str(report_metrics.get("report_text", "")).strip()
    cm_arr = np.asarray(report_metrics.get("confusion_matrix", []), dtype=np.int64)
    cm_text = np.array2string(cm_arr, separator=" ")
    if report_text:
        LOGGER.info("分类报告:\n%s", report_text)
    if cm_arr.size > 0:
        LOGGER.info("混淆矩阵:\n%s", cm_text)
    ...
    save_markdown_report(
        reports_dir=run_paths.reports_dir,
        file_name="report_attention_fusion.md",
        title="融合方式: attention",
        test_accuracy=float(val_acc),
        macro_f1=float(val_f1),
        report_text=report_text,
        confusion_image=str(report_metrics.get("confusion_image", "")),
