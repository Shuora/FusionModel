...
from fusion_common import add_common_args, build_common_kwargs, ensure_output_dirs, run_fusion_experiment, run_stacking_experiment, set_seed

def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Run attention-based CharBERT + MobileViT experiments')
    p.add_argument('--mode', choices=['attention', 'attention_stacking', 'stress_test', 'all'], default='all', help='Which attention-based experiment to run')
    add_common_args(p)
...
def main() -> int:
    ...
    if args.mode == 'stress_test':
        ...
        for task in tasks:
            ...
            run_stacking_experiment(base_fusion_mode='attention', meta_methods=['xgboost'], ensemble_tag='attention_stacking', **kwargs)
        return 0
    kwargs = build_common_kwargs(args)
    ensure_output_dirs(kwargs['output_dir'])
    start = time.time()
