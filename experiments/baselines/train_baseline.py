...
from src.fusion_common import add_common_args, build_common_kwargs, load_fusion_data, evaluate_full, setup_logging, ensure_output_dirs, prepare_run_output_dir, build_run_artifact_paths, plot_training_curves, plot_confusion, save_report_md, export_metrics_artifacts, log_saved
...
def main():
    ...
    (train_loader, train_classes) = load_fusion_data(kwargs['train_image_dir'], kwargs['train_pcap_dir'], batch_size=kwargs['batch_size'], is_train=True, image_mode=image_mode)
    (test_loader, _) = load_fusion_data(kwargs['test_image_dir'], kwargs['test_pcap_dir'], batch_size=kwargs['batch_size'], is_train=False, image_mode=image_mode)
    ...
    (model, history) = train_model(model, train_loader, test_loader, epochs=kwargs['epochs'], lr=kwargs['lr'], device=device, patience=kwargs['patience'], output_dir=run_dir, tag=tag)
    ...
    save_report_md(artifact_paths['report_md'], title=f'Baseline: {args.model_type}', acc=eval_result['acc'], macro_f1=eval_result['macro_f1'], report=eval_result['report'], cm=eval_result['cm'], confusion_image=artifact_paths['confusion_matrix'].name, curve_image=artifact_paths['metrics_curve'].name)
