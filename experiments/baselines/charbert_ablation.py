...
class CharBERTAblation(nn.Module):
    ...
    def __init__(self, num_classes=10, char_hidden_size=128):
        super().__init__()
        self.text_encoder = CharBERTTextEncoder(feature_dim=char_hidden_size, seq_len=784, hidden_size=char_hidden_size, num_layers=2, num_heads=4, dropout=0.3)
        self.classifier_head = nn.Sequential(nn.Linear(char_hidden_size, 512), nn.ReLU(), nn.Dropout(0.3), nn.Linear(512, num_classes))
